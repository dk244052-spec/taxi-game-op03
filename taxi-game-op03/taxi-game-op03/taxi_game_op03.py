...
def open_rules():
    ...
    text += "ОБОЗНАЧЕНИЯ:\n"
    text += "T - такси (пустое)\n"
    text += "X - такси с пассажиром\n"
    text += "P - пассажир\n"
    text += "D - пункт назначения\n"
    text += ". - пустая клетка\n\n"
    text += "ПРАВИЛА:\n1. Найди P\n2. Вези к D\n3. Получи +50 денег"
...
def open_stats():
    ...
    label_title = tk.Label(stats_win, text="СТАТИСТИКА ИГРЫ", font=("Arial", 12, "bold"))
    label_title.pack(pady=5)
    
    text_area = tk.Text(stats_win, height=15)
    text_area.pack(fill="both", expand=True, padx=10, pady=5)
    
    text_area.insert("end", "Здесь будет лог-файл с действиями")
    text_area.config(state="disabled")
    
    btn = tk.Button(stats_win, text="Закрыть", command=stats_win.destroy)
    btn.pack(pady=5)
...
def start_game():
    ...
    btn_up = tk.Button(control_frame, text="Вверх", width=8, 
                      command=lambda: move("up"))
    btn_up.grid(row=0, column=1, padx=2, pady=2)
    
    btn_left = tk.Button(control_frame, text="Влево", width=8,
                        command=lambda: move("left"))
    btn_left.grid(row=1, column=0, padx=2, pady=2)
    
    btn_down = tk.Button(control_frame, text="Вниз", width=8,
                        command=lambda: move("down"))
    btn_down.grid(row=1, column=1, padx=2, pady=2)
    
    btn_right = tk.Button(control_frame, text="Вправо", width=8,
                         command=lambda: move("right"))
    btn_right.grid(row=1, column=2, padx=2, pady=2)
    
    btn_menu = tk.Button(game_win, text="В главное меню", bg="lightgray",
